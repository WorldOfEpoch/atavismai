...
//                Orientation = Quaternion.Lerp(Orientation, _orientation, timeSinceLastFrame * AtavismClient.Instance.smoothRotation);
        ...
        public virtual void LateTick(float timeSinceLastFrame)
        {
            ...
            {
              //  string s = "";
              List<string> propsKeys = new List<string>();
            foreach (string prop in newProperties.Keys)
            {
                // set the property 
                if (prop.StartsWith("S_"))
                {
                    string _sid = prop.Substring(2);
                    int sid = int.Parse(_sid);
                    string stat = AtavismPrefabManager.Instance.GetStat(sid);
                    properties[stat] = newProperties[prop];
                    propsKeys.Add(stat);
                    properties[stat + "_t"] = Time.time;
          ...
          //  Debug.LogError("UpdateProperties name="+Name+" Params: "+s);
            foreach (string prop in propsKeys)
            {
        ...
        public object GetProperty(string propName)
        {
            ...
            AtavismLogger.LogWarning("No Property " + propName);
            return null;
