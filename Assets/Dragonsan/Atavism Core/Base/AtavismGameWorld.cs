...
namespace Atavism
{
    ...
    #region Message handlers
        ...
        private void HandleObjectPropertyHelper(long oid, Dictionary<string, object> props)
        {
            ...
            // if(props.ContainsKey("reaction"))
//           UnityEngine.Debug.LogError(DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss.fff tt ")+"Got stat update message for object: " + oid + " queue size "+ (objectPropertyQueue.ContainsKey(oid)? objectPropertyQueue[oid].Count.ToString():" none")+" keys="+keys);
            if (objectPropertyQueue.ContainsKey(oid))
            {
