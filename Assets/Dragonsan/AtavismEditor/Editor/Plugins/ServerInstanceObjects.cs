     ...
     //   public new float resultDuration = 5;
        ...
        void GetSceneInteractiveObjects()
        {
          ...
          //  int instanceID = ServerInstances.GetInstanceID(instance);
            ...
            AtavismLogger.LogDebugMessage("Interactive Objects saving found new=" + newNodes.Count + " exist=" + preexistingNodes.Count);
            ...
            string insertNodeQuery = "INSERT INTO interactive_object (name, instance, locX, locY, locZ, interactionType, interactionID, interactionData1,"
                    + "interactionData2, interactionData3, questReqID, interactTimeReq, coordEffect, respawnTime, "
                    +"profileId, despawnDelay, despawnTime, makeBusy, useLimit, minLevel, maxLevel, itemReq, itemCountReq,itemReqGet, currencyReq, currencyCountReq, currencyReqGet, gameObject)" + " values ";
            ...
            foreach (InteractiveObject obj in newNodes)
            {
                ...
                {
                    instanceId = AtavismEditorFunctions.GetInstanceID(obj.gameObject.scene.name);
                    if (instanceId == -1)
                    {
                        if (EditorUtility.DisplayDialog("Interactive Objects saving", "Not Found in Database Instance with name '"+obj.gameObject.scene.name+"'\n"+
                                "for object '"+obj.gameObject.name+"'\n"+
                                "Check Instance name before continue (Scene must have same name as instance)", "OK"))
                        {
                            return;
                        }
                    }
                ...
                {
                    instanceId = instanceList[obj.gameObject.scene.name];
                }
                ...
                string prefabPath = "";
                if(obj.prefab != null)
                prefabPath = AssetDatabase.GetAssetPath( obj.prefab );
                query += "('" + obj.name + "'," + instanceId + "," + obj.transform.position.x + "," + obj.transform.position.y
                    ...
                    + "','" + obj.interactionData2 + "','" + obj.interactionData3 + "'," + obj.questReqID + "," + obj.interactTimeReq 
                    + ",'" + coordEffect + "'," + obj.refreshDuration + "," + obj.profileId + "," + obj.despawnDelay + "," 
                    + obj.despawnTime + "," + obj.makeBusy + "," + obj.useLimit + "," + obj.minLevel +"," + obj.maxLevel 
                    + "," + obj.itemRequirement +"," + obj.itemCountRequirement + "," + obj.itemRequirementGet +"," + obj.currencyRequirement +"," + obj.currencyCountRequirement +"," + obj.currencyRequirementGet+ ",'"+prefabPath+"')";
            ...
            foreach (InteractiveObject obj in preexistingNodes)
            {
                ...
                query = "UPDATE interactive_object set name = '" + obj.name + "', locX = " + obj.transform.position.x
                    + ", locY = " + obj.transform.position.y + ", locZ = " + obj.transform.position.z + ", interactionType = '"
                        ...
                        + "', respawnTime = " + obj.refreshDuration + ", profileId = " + obj.profileId + ", despawnDelay = " + obj.despawnDelay 
                    + ", despawnTime = " + obj.despawnTime + ", makeBusy = " + obj.makeBusy + ", useLimit = " + obj.useLimit 
                    + ", minLevel = " + obj.minLevel + ", maxLevel = " + obj.maxLevel + ", itemReq = " + obj.itemRequirement+ ", itemReqGet = " + obj.itemRequirementGet 
                    + ", itemCountReq = " + obj.itemCountRequirement + ", currencyReq = " + obj.currencyRequirement  + ", currencyReqGet = " + obj.currencyRequirementGet 
                    + ", currencyCountReq = " + obj.currencyCountRequirement + ", gameObject='"+prefabPath+"' where id = " + obj.id;
              //  Debug.LogError(query);
                DatabasePack.Update(DatabasePack.contentDatabasePrefix, query, new List<Register>());
            }

            
            //Insert coordEffect lists
            string insertQuery = "INSERT INTO interactive_object_coordeffects (`objId`, `order`, `coordEffect`)" + " values ";
            insertCount = 0;
            query = insertQuery;
            foreach (InteractiveObject iObj in interactiveObjects)
            {
                
                Register delete2 = new Register("objId", "?objId", MySqlDbType.Int32, iObj.id.ToString(), Register.TypesOfField.Int);
                DatabasePack.Delete(DatabasePack.contentDatabasePrefix, "interactive_object_coordeffects", delete2, true);

                int c = 1;
                foreach (var ocoordEffect in iObj.activateCoordEffects)
                {
                    string coordEffect = "";
                    if (ocoordEffect != null)
                        coordEffect = ocoordEffect.name;
                    if (coordEffect.Length > 0)
                    {
                        if (insertCount > 0)
                            query += ",";
                        query += "(" + iObj.id + "," + c + ",'" + coordEffect +"')";
                        c++;
                        insertCount++;
                    }
                    if (insertCount == bulkEntryCount)
        ...
        void ClearSavedInstanceInteractiveObjectsData()
        {
            ...
            for (int i = 0; i < EditorSceneManager.sceneCount; i++)
            {
                ...
                {
                    int instanceID = AtavismEditorFunctions.GetInstanceID(EditorSceneManager.GetSceneAt(i).name);
        ...
        public static Dictionary<string, string> LoadInteractiveObjectProfileCoords(int profileId, Dictionary<string, string> list)
        {
            // Read all entries from the table
        ...
        void GetSceneRegions()
        {
           ...
           // int instanceID = ServerInstances.GetInstanceID(instance);
            ...
            {
                int instanceId = AtavismEditorFunctions.GetInstanceID(region.gameObject.scene.name);
        ...
        void GetSceneGraveyards()
        {
          ...
          //  int instanceID = ServerInstances.GetInstanceID(instance);
            ...
            {
                int instanceId = AtavismEditorFunctions.GetInstanceID(gy.gameObject.scene.name);
