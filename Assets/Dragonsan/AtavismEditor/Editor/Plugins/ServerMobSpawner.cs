...
namespace Atavism
{
    ...
    public class ServerMobSpawner : AtavismFunction
    {
        ...
        public override void Draw(Rect box)
        {
            ...
            pos.y += ImagePack.fieldHeight * 1.1F ;
            ImagePack.DrawText(pos, Lang.GetTranslate("Reset Spawn Id in the Mob Spawn Marker in the current scene by clicking below."));
            pos.y += ImagePack.fieldHeight;
            if (ImagePack.DrawButton(pos.x, pos.y, Lang.GetTranslate("Reset Spawn Id ")))
            {
                ResetSpawnIdSceneMobSpawnMarker();
            }
            pos.y += ImagePack.fieldHeight * 1.1F ;
            ...
            if (ImagePack.DrawButton(pos.x, pos.y, Lang.GetTranslate("Load")))
            {
                ...
                for(int i=0;i<SceneManager.sceneCount;i++ )
                {
                    ...
                    if (s.isLoaded)
                    {
                        int instanceID = AtavismEditorFunctions.GetInstanceID(s.name);
                        if (instanceID > 0)
                        {
                            ...
                            List<MobSpawnData> list =  AtavismEditorFunctions.LoadMobSpawnDataEntities(instanceID);
                        //    Debug.LogError("Loaded Mobs For instance "+instanceID+" "+s.name+" count "+list.Count);
                            foreach (var msd in list)
                            {
                                Mob mob = AtavismEditorFunctions.LoadMobTemplateModel(msd.mobTemplate);
                                ...
                                amsm.roamDelayMin = msd.roamDelayMin;
                                amsm.roamDelayMax = msd.roamDelayMax;
                                amsm.roamRollTimeEachTime = msd.roamRollTimeEachTime;
        ...
        //    ImagePack.DrawText(pos, Lang.GetTranslate("When the Mob Spawn Marker have been saved, save your scene again."));
            ...
            if (showConfirmDelete)
            {
                ...
                if (ImagePack.DrawButton(pos.x + 150, pos.y, Lang.GetTranslate("Cancel")))
                {
                    showConfirmDelete = false;
                }
            }
        ...
        void GenerateSceneMobSpawnMarker()
        {
            ...
            foreach (UnityEngine.Object spawnersspawnerObj in spawners)
            {
                ...
                if (spawner != null)
                {
                    Mob mob = AtavismEditorFunctions.LoadMobTemplateModel(spawner.mobTemplateID);
        ...
        void ResetSpawnIdSceneMobSpawnMarker()
        {
            UnityEngine.Object[] spawners = FindObjectsOfType<AtavismMobSpawnMarker>();
            string query = "";
            foreach (UnityEngine.Object spawnersspawnerObj in spawners)
            ...
            NewResult(Lang.GetTranslate("Spawn Id for Mob Spawn Markers reset"));
        }
        ...
        void GetSceneMobSpawnMarker()
        {
            ...
            string insertNodeQuery = "INSERT INTO spawn_data (category,name,mobTemplate,mobTemplate2,mobTemplate3,mobTemplate4,mobTemplate5,markerName,locX,locY,locZ,orientX,orientY,orientZ,orientW,instance,numSpawns,"
                                     ...
                                     +",alternateSpawnMobTemplate5,combat,roamRadius,startsQuests,endsQuests,startsDialogues,otherActions,baseAction,weaponSheathed,merchantTable,patrolPath,questOpenLootTable,isChest,pickupItem,roamDelayMin,roamDelayMax,roamRollTimeEachTime) values ";
            ...
            {
                int instanceID = AtavismEditorFunctions.GetInstanceID(sm.gameObject.scene.name);
                if (instanceID > 0)
                {
                    ...
                    if (go != null)
                    {
                        ...
                        query += "(1,'" + sm.name + "'," + sm.mobTemplateID + "," + sm.mobTemplateID2 + "," + sm.mobTemplateID3 + "," + sm.mobTemplateID4 + "," + sm.mobTemplateID5
                                 ...
                                 + baseAction + "',0," + sm.merchantTable + "," + sm.patrolPath + ",-1," + sm.isChest + "," + sm.pickupItemID +"," + sm.roamDelayMin +"," + sm.roamDelayMax +"," + sm.roamRollTimeEachTime + ")";
                        insertCount++;
            ...
            {
                int instanceId = AtavismEditorFunctions.GetInstanceID(obj.gameObject.scene.name);
                ...
                string baseAction = "";
                    ...
                    query = "UPDATE `spawn_data` SET `mobTemplate`=" + obj.mobTemplateID + ",`mobTemplate2`=" + obj.mobTemplateID2 + ",`mobTemplate3`=" + obj.mobTemplateID3 + ",`mobTemplate4`=" + obj.mobTemplateID4 + ",`mobTemplate5`=" + obj.mobTemplateID5 + "," +
                            ...
                            "`roamDelayMin`=" + obj.roamDelayMin + ", `roamDelayMax`=" + obj.roamDelayMax + ", `roamRollTimeEachTime`=" + obj.roamRollTimeEachTime + 
               ...
               /* if (!instanceList.ContainsKey(obj.gameObject.scene.name))
                ...
                else
                {
                    ...
                    insertQuery += "(1,'" + obj.name + "'," + obj.mobTemplateID + "," + obj.mobTemplateID2 + "," + obj.mobTemplateID3 + "," + obj.mobTemplateID4 + "," + obj.mobTemplateID5 + ",''," + obj.transform.position.x + "," + obj.transform.position.y + "," + obj.transform.position.z
                                   ...
                                   + ",1," + obj.roamRadius + ",'" + startsQuests + "','" + endsQuests + "','" + startsDialogues + "','" + otherActions + "','" + baseAction + "',0," + obj.merchantTable + "," + obj.patrolPath + ",-1," + obj.isChest + "," + obj.pickupItemID +"," + obj.roamDelayMin +"," + obj.roamDelayMax +"," + obj.roamRollTimeEachTime+ ")";
        ...
        void ClearSavedInstanceNodeData()
        {
            ...
            for (int i = 0; i < EditorSceneManager.sceneCount; i++)
            {
                if (EditorSceneManager.GetSceneAt(i).isLoaded)
                {
                    ...
                    Register delete2 = new Register("instance", "?instance", MySqlDbType.Int32, instanceID.ToString(),
                        Register.TypesOfField.Int);
                    DatabasePack.Delete(DatabasePack.contentDatabasePrefix, "spawn_data", delete2, true);
                }
            }
