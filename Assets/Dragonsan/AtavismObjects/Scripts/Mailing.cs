...
namespace Atavism
{
    ...
    public class Mailing : MonoBehaviour
    {
        ...
        public void SetMailCurrencyAmountConvert(int currencyID, long amount)
        {
            var currencies =  Inventory.Instance.GetConvertedCurrencyValues(currencyID, amount);
            foreach (var v in currencies)
            {
                if(mailBeingComposed!=null)
                mailBeingComposed.currencies[Inventory.Instance.GetCurrency((int)v.x)] =(int) v.y;  
            }
