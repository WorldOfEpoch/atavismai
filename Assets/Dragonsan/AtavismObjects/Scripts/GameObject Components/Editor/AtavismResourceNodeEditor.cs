...
namespace Atavism
{
    ...
    public class AtavismResourceNodeEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!profilesLoaded)
            {
                AtavismEditorFunctions.LoadResourceNodeProfileOptions(true);
                profilesLoaded = true;
            }
            ...
            profileIdProp.intValue = AtavismEditorFunctions.GetFilteredListSelector(content, ref search, profileIdProp.intValue, AtavismEditorFunctions.RNProfileList, AtavismEditorFunctions.RNProfileIds);
            if (help)
                EditorGUILayout.HelpBox(content.tooltip, MessageType.None);
            ...
            for (int i = 0; i < obj.subProfileGameObjects.Count; i++)
            {
                ...
                obj.subProfileGameObjects[i] = (GameObject) EditorGUILayout.ObjectField(content,obj.subProfileGameObjects[i], typeof(GameObject), false);
                if (help)
                    EditorGUILayout.HelpBox(content.tooltip, MessageType.None);
