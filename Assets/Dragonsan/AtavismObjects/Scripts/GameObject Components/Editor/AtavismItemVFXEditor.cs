          ...
          //  var indentOffset = EditorGUI.indentLevel * 5f;
            ...
            for (int j = 0; j < obj.socketEffects.Count; j++)
            {
                ...
                for (int i = 0; i < obj.socketEffects[j].items.Count; i++)
                {
                    ...
                    obj.socketEffects[j].items[i] = AtavismEditorFunctions.GetFilteredListSelector(content, ref search, obj.socketEffects[j].items[i], AtavismEditorFunctions.GuiItemsList, AtavismEditorFunctions.itemIds);
