...
namespace Atavism
{
    ...
    public class AtavismTrade : MonoBehaviour
    {
        ...
        public void SetCurrencyAmountConvert(int currencyID, int amount)
        {
            var currencies =  Inventory.Instance.GetConvertedCurrencyValues(currencyID, amount);
            foreach (var v in currencies)
            {
                myCurrencyOffers[((int)v.x).ToString()] =(int) v.y;  
            }
            SendTradeOffer();
        }
        ...
        public void HandleTradeRequest(Dictionary<string, object> props)
        {
            ...
            // dispatch a ui event to tell the rest of the system
#if AT_I2LOC_PRESET
            if(UGUIConfirmationPanel.Instance!=null) UGUIConfirmationPanel.Instance.ShowConfirmationBox(ClientAPI.GetObjectNode(partnerOid).Name + " " + I2.Loc.LocalizationManager.GetTranslation("has invited you to trade"), tradePartnerOid, TradeInviteResponse, count);
            if(UIAtavismConfirmationPanel.Instance!=null) UIAtavismConfirmationPanel.Instance.ShowConfirmationBox(ClientAPI.GetObjectNode(partnerOid).Name + " " + I2.Loc.LocalizationManager.GetTranslation("has invited you to trade"), tradePartnerOid, TradeInviteResponse, count);
#else
            ...
            if(UIAtavismConfirmationPanel.Instance!=null)
                UIAtavismConfirmationPanel.Instance.ShowConfirmationBox(ClientAPI.GetObjectNode(partnerOid).Name + " has invited you to trade", tradePartnerOid, TradeInviteResponse, count);
#endif
...
#endif
                if(UGUIConfirmationPanel.Instance!=null) UGUIConfirmationPanel.Instance.Hide();
                if(UIAtavismConfirmationPanel.Instance!=null) UIAtavismConfirmationPanel.Instance.Hide();
