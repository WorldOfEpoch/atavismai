...
/*   [System.Flags]
  ...
  public struct OptionChoice
  {
    ...
    public class WorldBuilder : MonoBehaviour
    {
        ...
        public void StartPlaceClaimObject(UIAtavismActivatable activatable)
        {
        ...
        {
            Debug.LogError("GetBuildObjectsOfCategory "+category+" "+nonItemsOnly);
        ...
        {
            // Debug.LogError("GetBuildObjectsOfObjectCategory "+category+" "+nonItemsOnly+" "+(ActiveClaim != null?ActiveClaim.claimType:"No claim type"));
            List<AtavismBuildObjectTemplate> buildObjects = new List<AtavismBuildObjectTemplate>();
            foreach (AtavismBuildObjectTemplate template in buildObjectTemplates.Values)
            {
                // Debug.LogError("GetBuildObjectsOfCategory T="+template.buildObjectName+" TCategory="+string.Join(",",template.objectCategory)+" category="+category+" TClaimT="+string.Join(",",template.validClaimTypes) +" Limits count="+(ActiveClaim!=null?ActiveClaim.limits.Count+"":"BD"));
        ...
        private void ClaimTaxPaymentMessage(Dictionary<string, object> props)
        {
          ...
          //  Debug.LogError("ClaimTaxPamentMessage claimID="+claimID+" currency="+currency+" amount="+amount+" time="+time);
            ...
            if(UIAtavismConfirmationPanel.Instance!=null)
                UIAtavismConfirmationPanel.Instance.ShowConfirmationBox(msg, claimID, PayTaxResponse);
        ...
        public void MoveClaimObjectConfirmMessage(Dictionary<string, object> props)
        {
            ...
            if(UIAtavismConfirmationPanel.Instance!=null)
                UIAtavismConfirmationPanel.Instance.ShowConfirmationBox(msg, objectID, MoveObjectResponse);
        ...
        public void RemoveClaimObjectConfirmMessage(Dictionary<string, object> props)
        {
            ...
            if(UIAtavismConfirmationPanel.Instance!=null)
                UIAtavismConfirmationPanel.Instance.ShowConfirmationBox(msg, objectID, DeleteObjectResponse);
            ...
            try
            {
                ...
                for (int i = 0; i < num; i++)
                {
                   // Debug.LogError("HandleInvCurrPrefabData " + i);
                    ...
                    string objCat = (string)props["i" + i + "objCat"];
                    if (objCat.Length > 0)
                    {
