...
namespace Atavism
{
    public class GroupMember
    {
        ...
        public float GetPropertyStatWithPrecision(string propName)
        {
            if (properties.ContainsKey(propName))
            {
                return AtavismPrefabManager.Instance.GetStatValueWithPrecision(propName, (int)properties[propName]);
    ...
    public class AtavismGroup : MonoBehaviour
    {
        ...
        int grade = 0;
        OID mob = null;
        private OID groupRequestInviterOid; // BeWise edited
        ...
        public void ConfirmGroupRequestInvitation()
        {
            SendInviteResponseMessage(groupRequestInviterOid, "accept");
        }

        public void DeclineGroupRequestInvitation()
        {
            SendInviteResponseMessage(groupRequestInviterOid, "decline");
        }
...
#if AT_I2LOC_PRESET
        UIAtavismConfirmationPanel.Instance.ShowConfirmationBox(groupLeaderName + " " + I2.Loc.LocalizationManager.GetTranslation("has invited you to join their group"), groupLeaderOid, GroupInviteResponse,(float)count);
#else
                UIAtavismConfirmationPanel.Instance.ShowConfirmationBox(groupLeaderName + " " + "has invited you to join their group", groupLeaderOid, GroupInviteResponse, (float)count);
#endif
