...
namespace Atavism
{
    ...
    public class Inventory : MonoBehaviour
    {
        ...
        Dictionary<int, AtavismInventoryItem> equippedPetItems = new Dictionary<int, AtavismInventoryItem>();
        ...
        void Start()
        {
            ...
            NetworkAPI.RegisterExtensionMessageHandler("EquippedPetInventoryUpdate", HandleEquippedPetInventoryUpdate);
            ...
            NetworkAPI.RegisterExtensionMessageHandler("item_broken", HandleItemBroken);
        }
        ...
        private void OnDestroy()
        {
            ...
            AtavismClient.Instance.NetworkHelper.RemovePrefabMessageHandler("SlotsProfile", HandleSlotsProfilePrefabData);
        ...
        void ClientReady()
        {
            ...
            AtavismClient.Instance.NetworkHelper.RegisterPrefabMessageHandler("SlotsProfile", HandleSlotsProfilePrefabData);
        ...
        public void EquipItemInPetSlot(AtavismInventoryItem item, string slotName,int profile)
        {
            Dictionary<string, object> props = new Dictionary<string, object>();
            props.Add("itemOid", item.ItemId);
            props.Add("slotName", slotName);
            props.Add("profile", profile);
            NetworkAPI.SendExtensionMessage(0, false, "ao.EQUIP_ITEM_IN_PET_SLOT", props);
        }
...
/// <summary>
/// Generates a list of currencies from main Currency Group
/// </summary>
/// <returns></returns>
        public List<CurrencyDisplay> GenerateMainCurrencyList()
        {
            List<CurrencyDisplay> generatedCurrencyList = new List<CurrencyDisplay>();
            List<Currency> cur = GetMainCurrencies();
            foreach (var c in cur)
            {
                if (c != null)
                {
                    CurrencyDisplay currencyDisplay = new CurrencyDisplay();
                    currencyDisplay.icon = c.Icon;
                    currencyDisplay.name =  GetCurrencyName(c.id);;
               ...
               // Debug.LogError("Inventory GetCost =" + currencyValues.Count);
                ...
                for (int i = 0; i < currencyValues.Count; i++)
                {
                  //  Debug.LogError("Inventory GetCost =" + currencyValues[i]);
                    Currency _c = GetCurrency((int)currencyValues[i].x);
                    CurrencyDisplay currencyDisplay = new CurrencyDisplay();
                    currencyDisplay.icon = _c.Icon;
...
public void HandleInvCurrIcon(Dictionary<string, object> props)
       ...
       public void HandleSlotsProfilePrefabData(Dictionary<string, object> props)
        {
            if(AtavismLogger.isLogDebug())
                AtavismLogger.LogDebugMessage("HandleSlotsProfilePrefabData " + Time.time);
            int tmplId = 0;
            try
            {
                int num = (int)props["num"];
                ...
                {
                    if(AtavismLogger.isLogDebug())
                        AtavismLogger.LogDebugMessage("HandleSlotsProfilePrefabData " + i);
                    SlotsProfileData spd = new SlotsProfileData();
                    spd.id = (int)props["i" + i + "id"];
                    spd.name = (string)props["i" + i + "name"];
                    spd.date = (long)props["i" + i + "date"];
                    int slots_num = (int)props["i" + i + "num"];
                    for (int j = 0; j < slots_num; j++)
                    {
                        spd.slots.Add((string)props["i" + i + "s" + j]);      
                    }

                    AtavismPrefabManager.Instance.SaveSlotsProfile(spd);
                }
                ...
                if (props.ContainsKey("toRemove"))
                {
                    ...
                    if (keys.Length > 0)
                    {
                        ...
                        foreach (string k in _keys)
                        {
                            if (k.Length > 0)
                            {
                                AtavismPrefabManager.Instance.DeleteSlotsProfile(int.Parse(k));
                            }
                        }
                    }
                }
                ...
                {
                    if(AtavismLogger.isLogDebug())
                        AtavismLogger.LogDebugMessage("All data received for slots Profiles update message.");
                    AtavismPrefabManager.Instance.reloaded++;
                }
                else
                {
                    AtavismPrefabManager.Instance.LoadWeaponProfilesPrefabData();
                    if(AtavismLogger.isLogDebug())
                        AtavismLogger.LogDebugMessage("Not all slots profile data was sent by Prefab server");
                }
            }
            catch (System.Exception e)
            {
                AtavismLogger.LogError("Exception loading slots profile prefab data item id=" + tmplId + " Exception:" + e);
            }

            if (AtavismLogger.logLevel <= LogLevel.Debug)
                Debug.Log("HandleSlotsProfilePrefabData End");
        }
        ...
        public void HandleBagInventoryUpdate(Dictionary<string, object> props)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < numBags; i++)
                {
                    ...
                    int tId = (int)props["bag_" + i + "TId"];

                    AtavismInventoryItem invInfo = GetItemByTemplateID(tId);
        ...
        public void HandleEquippedInventoryUpdate(Dictionary<string, object> props)
        {
            ...
            public void HandleEquippedPetInventoryUpdate(Dictionary<string, object> props)
        {
            if(AtavismLogger.isLogDebug())
                AtavismLogger.LogDebugMessage("HandleEquippedPetInventoryUpdate start");
                    ...
                    //     keys += " ; " + it + " => " + props[it];
                    // }
                    // Debug.LogWarning("HandleEquippedPetInventoryUpdate: keys:" + keys);
                    //
            List<int> iconLack = new List<int>();
               foreach (AtavismInventoryItem aii in equippedPetItems.Values)
               {
                pool.Enqueue(aii);
               }
            equippedPetItems.Clear();
            try
            {
                if(props.ContainsKey("setSelected"))
                    setSelected = (string)props["setSelected"];
                int numSlots = (int)props["numSlots"];
                if(AtavismLogger.isLogDebug())
                    AtavismLogger.LogDebugMessage("HandleEquippedPetInventoryUpdate numSlots="+numSlots);
                for (int i = 0; i < numSlots; i++)
                {
                    if (!props.ContainsKey("item_" + i + "Name"))
                        continue;
                    ...
                    AtavismInventoryItem invInfo = LoadItemPrefabData(baseName);
                    if(AtavismLogger.isLogDebug())
                        AtavismLogger.LogDebugMessage("HandleEquippedPetInventoryUpdate name="+name+" baseName=" + baseName + " invInfo=" + invInfo);
                    if (invInfo.Icon == null)
                    {
                        iconLack.Add(invInfo.templateId);
                    }
                    ...
                    invInfo.MaxDurability = (int)props["item_" + i + "MaxDurability"];
                    if(AtavismLogger.isLogDebug())
                        AtavismLogger.LogDebugMessage("HandleEquippedPetInventoryUpdate name="+name+" baseName=" + baseName + " slot=" + invInfo.slot+" id "+invInfo.ItemId);
                    if (invInfo.MaxDurability > 0)
                    {
                        invInfo.Durability = (int)props["item_" + i + "Durability"];
                        //AtavismLogger.LogDebugMessage("Durability: " + invInfo.Durability + "/" + invInfo.MaxDurability);
                    ...
                    equippedPetItems.Add(i, invInfo);
                    if(AtavismLogger.isLogDebug())
                        AtavismLogger.LogDebugMessage("Added equipped item: " + invInfo.name + " to slot: " + i);
                }
            }
            ...
            {
                Debug.LogError("HandleEquippedPetInventoryUpdate Bag Exception:" + e);
            }
            // Get Equipped Ammo
            // For now Pet dont have ammo
            // int equippedAmmoID = (int)props["equippedAmmo"];
            // if (equippedAmmoID > 0)
            // {
            //     equippedAmmo = GetItemByTemplateID(equippedAmmoID);
            ...
            if (iconLack.Count > 0)
            {
                ...
                ps.Add("objs", s);
                AtavismClient.Instance.NetworkHelper.GetIconPrefabs(ps, "ItemIcon");
                //Debug.LogError("HandleEquippedPetInventoryUpdate " + Time.time);
            }
            string[] args = new string[1];
            AtavismEventSystem.DispatchEvent("PET_INVENTORY_UPDATE", args);
            if(AtavismLogger.isLogDebug())
                AtavismLogger.LogDebugMessage("HandleEquippedPetInventoryUpdate END");
...
#region Properties
        ...
        */
        ...
        public Dictionary<int, AtavismInventoryItem> EquippedPetItems
        {
            get
            {
                return equippedPetItems;
            }
        }
