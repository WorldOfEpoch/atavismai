    ...
    public class CharacterSelectionCreationManager : MonoBehaviour
    {
      ...
      //  public Texture cursorOverride;
        ...
        void Update()
        {
            ...
            {
                Vector3 destPosIn = character.GetComponent<AtavismMobAppearance>().GetSocketTransform("Head").position + cameraInLoc;
                Vector3 destPosOut = character.transform.position + cameraOutLoc;
        ...
        void LateUpdate()
        {
            if (character && Input.GetMouseButton(0) && !AtavismCursor.Instance.IsMouseOverUI() )
            {
        ...
        public void StartCharacterSelection()
        {
            ...
            if (characterEntries.Count > 0)
            {
                int index = PlayerPrefs.GetInt("LAST_SELECTED_CHARACTER_INDEX", 0);
                if (index > characterEntries.Count)
                    index = characterEntries.Count - 1;
                CharacterSelected(characterSelected!=null? characterSelected:characterEntries[index]);
                if (enterUI != null)
                {
        ...
        public virtual void CharacterSelected(CharacterEntry entry)
        {
            ...
            int index = characterEntries.IndexOf(characterSelected);
            if (index != -1)
            {
