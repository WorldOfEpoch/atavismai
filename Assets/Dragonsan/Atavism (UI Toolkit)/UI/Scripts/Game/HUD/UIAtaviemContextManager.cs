...
namespace Atavism.UI
{
    ...
    public class UIAtavismContextManager : MonoBehaviour
    {
        [SerializeField] UIDocument uiDocument;
        public VisualTreeAsset contextPrefab;
        ...
        Dictionary<int, UIAtavismContextPrefab> contextObjects = new Dictionary<int, UIAtavismContextPrefab>();

        private UIAtavismContextPrefab closestContext = null;
        [HideInInspector] public AtavismNode closestContextNode; //Used for caching the node   

        public VisualElement contextCanvas;
        ...
        private void RegisterUI()
        {
            ...
            contextCanvas = uiDocument.rootVisualElement;
        }
        ...
        void Update()
        {
            ...
            if (closestContextNode != null && closestContext != null && closestContext.contextInfo != null)
            {
                ...
                if (closestContext != null && closestContext.contextInfo.parentUiObject != null)
                {
                    if (closestContext.interactFiller != null)
                    {
                        closestContext.interactFiller.value = Mathf.Lerp(0, 1, lastInteractTime / closestContext.contextInfo.interactTime);
                    }
                }
            }
        ...
        void UpdateContext(UIAtavismContextPrefab contextPrefab)
        {
          //  Debug.LogError("UpdateContext");
            if (contextPrefab != null && contextPrefab.contextInfo != null)
            {
                ...
                if (sameAsClosestContextNode || contextPrefab.contextInfo.contextUpdateNeeded) //If it is the closest context node always update else only do it once.
                {
                    ...
                    contextPrefab.contectButton.text =
        ...
        IEnumerator checkSensorsAndRange(float waitTime)
        {
            while (true)
            {
                if (centerPointSensor != null && Camera.main != null)
                {
                    ...
                    Dictionary<int, UIAtavismContextPrefab> contexts = new Dictionary<int, UIAtavismContextPrefab>(contextObjects);
                    ...
                    UIAtavismContextPrefab tempPrefab = null;

                    foreach (Collider hitCollider in hitColliders)
                    {
                        if (hitCollider != null && hitCollider.gameObject != null)
                        {
                            UIAtavismContextPrefab context = null;
                            ...
                            else
                            {
                                ...
                                if (contextInfo != null)
                                {
                                    if (contextInfo.parentUiObject == null && contextPrefab != null && contextCanvas != null)
                                    {
                                        VisualElement v = contextPrefab.Instantiate();
                                        context = new UIAtavismContextPrefab(v);
                                        v.userData = context;
                                        contextCanvas.Add(context.m_Root);
                                        contextInfo.parentUiObject = context;
                            ...
                            if (context != null && contextInfo != null)
                            {
                                ...
                                if (context.m_Root.parent != null && !focusedOverwrite)
                                {
                    ...
                    if (tempPrefab == null)
                    {
                        if (closestContext != null)
                        {
                            if (closestContext.interactFiller != null)
                            {
                                closestContext.interactFiller.value = 0f;
                            }
                    ...
                    foreach (KeyValuePair<int, UIAtavismContextPrefab> keyValuePair in contexts)
                    {
                        UIAtavismContextPrefab context = keyValuePair.Value;
                        if (context != null)
                        {
                            context.playerInRange = false;
                            context.contextInfo.parentUiObject.Delete();
                            context.contextInfo.parentUiObject = null;
        ...
        static UIAtavismContextManager instance;

        public static UIAtavismContextManager Instance
        {
            get { return instance; }
        }
    }
