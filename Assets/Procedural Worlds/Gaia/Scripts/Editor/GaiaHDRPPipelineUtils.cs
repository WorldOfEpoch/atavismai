...
namespace Gaia.Pipeline.HDRP
{
    ...
    public static class GaiaHDRPPipelineUtils
    {
        ...
        {
            if (GraphicsSettings.defaultRenderPipeline == null || EditorUtility.DisplayDialog("Use Gaia Render Pipeline Asset?", "HDRP requires a render pipeline asset to be set up in the Project Graphics Settings. It looks like you already have a render pipeline asset in place. You can either use Gaias render pipeline asset with preconfigured settings (Recommended) or keep the already existing pipeline asset in place.", "Use Gaias RP Asset", "Keep existing RP Asset"))
            {
