...
public class SnowManagerEditor : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		if (GUILayout.Button("Find All Renderers in scene"))
		{
			t.renderers.Clear();
			Renderer[] list = GameObject.FindObjectsOfType<Renderer>();
			Debug.LogError("Find renderer "+list.Length);
			foreach (var r in list)
			{
				Debug.LogError("Find renderer "+r.gameObject.scene.name+" "+t.gameObject.scene.name);
				if (r.gameObject.scene.name.Equals(t.gameObject.scene.name))
				{
					bool found = false;
					foreach (var m in t.materials)
					{
						if (r.sharedMaterials.Contains(m))
						{
							found= true;
						}	
					}
					...
					if (found)
					{
						Debug.LogError("Find renderer add");
						t.renderers.Add(r);
					}
				}
			}

			t.renderersSize = t.renderers.Count;
		}
		EditorGUILayout.LabelField ("Renderers:", EditorStyles.boldLabel);
		EditorGUI.indentLevel += 1;
		t.renderersCollapsed = EditorGUILayout.Foldout (t.renderersCollapsed, "Renderers " + t.renderersSize + ": ");
		if (t.renderersCollapsed) {
			EditorGUI.indentLevel++;
			t.renderersSize = EditorGUILayout.IntField ("size", t.renderersSize);
			if (t.renderersSize != t.renderers.Count) {
				Undo.RecordObject (t, "Material size change");
				while (t.renderersSize > t.renderers.Count) {
					
					t.renderers.Add (null);
				}
				while (t.renderersSize < t.renderers.Count) {
					t.renderers.RemoveAt (t.renderers.Count - 1);
				}
			}
			for (int i = 0; i < t.renderers.Count; i++) {
				t.renderers [i] = (Renderer)EditorGUILayout.ObjectField ("Renderer:", t.renderers [i], typeof(Renderer), false);
			}
			EditorGUI.indentLevel--;
		}
