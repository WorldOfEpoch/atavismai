...
namespace AwesomeTechnologies.TouchReact
{
    ...
    [AddComponentMenu("AwesomeTechnologies/TouchReact/TouchReactSystem", 0)]
    [ExecuteInEditMode]
    public class TouchReactSystem : MonoBehaviour
    {
        ...
        private void OnDisable()
        {
            ...
            if (TouchReactCamera)
            {
                ...
                if (_currentTouchRenderTexture)
                    DestroyImmediate(_currentTouchRenderTexture);

                if (_lastFrameTouchRenderTexture)
                    DestroyImmediate(_lastFrameTouchRenderTexture);
            }
        }
        ...
        private void CreateTouchReactCamera()
        {
            ...
            if (tourchBendCameraTransform == null)
            {
                ...
                touchReactCameraObject.transform.SetPositionAndRotation(Vector3.zero, Quaternion.LookRotation(Vector3.up));
        ...
        {
            _touchReactMaterial = new Material(Shader.Find("AwesomeTechnologies/Development/TouchReact/RenderTouchBuffer")) { enableInstancing = true };
            _touchreactMaterialInstanced = new Material(Shader.Find("AwesomeTechnologies/Development/TouchReact/RenderTouchBufferInstanced")) { enableInstancing = true };
        ...
        public void UpdateCamera()
        {
            if (TouchReactCamera)
            {
                ...
                _currentTouchRenderTexture = new RenderTexture(textureResolution, textureResolution, 24, RenderTextureFormat.RGFloat)
                {
                ...
                _lastFrameTouchRenderTexture = new RenderTexture(textureResolution, textureResolution, 24, RenderTextureFormat.RGFloat)
                {
                    wrapMode = TextureWrapMode.Clamp,
                    filterMode = FilterMode.Point,
                    autoGenerateMips = false,
                ...
                _blendBufferMaterial = Resources.Load<Material>("BlendTouchReactBuffer");

                //RenderTexture oldRenderTexture = TouchReactCamera.targetTexture;

                // RenderTexture rt = new RenderTexture(textureResolution, textureResolution, 24, RenderTextureFormat.RGFloat, RenderTextureReadWrite.Linear)
                //{
                //    wrapMode = TextureWrapMode.Clamp,
                //    filterMode = FilterMode.Point,
                //    autoGenerateMips = false,
        ...
        {
            return touchReactQuality switch
            {
                TouchReactQuality.Low => 512,
                TouchReactQuality.Normal => 1024,
                TouchReactQuality.High => 2048,
                _ => 1024,
            };
        }
        ...
        void PrepareRenderTexture()
        {
            ...
            GL.Clear(true, true, TouchReactCamera.backgroundColor, 1);
        ...
        {
            for (int i = 0; i < _capsuleColliderMeshList.Count; i++)
        ...
        private void DrawSphereCollider(SphereCollider sphereCollider, float scale)
        {
            ...
            Matrix4x4 positionMatrix = Matrix4x4.TRS(sphereCollider.bounds.center, sphereCollider.transform.rotation, sphereScaleVector * sphereCollider.radius * scale);
        ...
        {
            Matrix4x4 positionMatrix = Matrix4x4.TRS(meshCollider.transform.position, meshCollider.transform.rotation, meshCollider.transform.lossyScale);
        ...
        {
            Matrix4x4 positionMatrix = Matrix4x4.TRS(meshfilter.transform.position, meshfilter.transform.rotation, meshfilter.transform.lossyScale);
...
#if UNITY_EDITOR
            if (TouchReactSceneViewDetector.GetCurrentSceneViewCamera())
                return TouchReactSceneViewDetector.GetCurrentSceneViewCamera().transform.position;
  ...
  return Vector3.zero;
        ...
        {
            if (ColliderList.Contains(touchColliderInfo) == false)
        ...
        public void InstanceDrawMeshInstanced(Mesh mesh, List<Matrix4x4> instanceList, int subMeshIndex)
        {
            ...
            {
                if (ShowDebugColliders)
                    Graphics.DrawMeshInstanced(mesh, subMeshIndex, _touchreactMaterialInstanced, instanceList, _touchreactMaterialPropertyBlock, ShadowCastingMode.Off, false, 0, null);

                for (int i = 0; i < instanceList.Count; i++)
                {
                    // Graphics.DrawMeshInstanced(mesh, subMeshIndex, _touchreactMaterialInstanced, instanceList, _touchreactMaterialPropertyBlock, ShadowCastingMode.Off, false,InvisibleLayer,TouchReactCamera);
        ...
        {
            Instance = FindAnyObjectByType<TouchReactSystem>();
        }
