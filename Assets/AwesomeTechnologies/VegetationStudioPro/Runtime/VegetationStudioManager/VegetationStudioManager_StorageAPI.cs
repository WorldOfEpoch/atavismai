    ...
    public partial class VegetationStudioManager
    {
        ...
        public static string AddVegetationItem(GameObject prefab, VegetationType vegetationType, bool enableRuntimeSpawn, BiomeType biomeType = BiomeType.Default)
        {
            ...
            if (Instance)
            {
                ...
                for (int i = 0; i < vegetationPackageList.Count; i++)
        ...
        public static void RestartVegetationSystem()
        {
            if (!Instance) FindInstance();
            if (Instance)
                for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                    Instance.VegetationSystemList[i].RestartVegetationSystem();
        }

        public static void RefreshItemSystem()
        {
            if (!Instance) FindInstance();
            if (Instance)
                for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                    Instance.VegetationSystemList[i].RefreshItemSystem();
        }
        ...
        public static string AddVegetationItem(Texture2D texture, VegetationType vegetationType, bool enableRuntimeSpawn, BiomeType biomeType = BiomeType.Default)
        {
            ...
            if (Instance)
            {
                ...
                List<VegetationPackagePro> vegetationPackageList = GetVegetationPackageList(biomeType);
                for (int i = 0; i < vegetationPackageList.Count; i++)
                    vegetationPackageList[i].AddVegetationItem(texture, vegetationType, enableRuntimeSpawn, newVegetationItemID);
                RefreshItemSystem();
                return newVegetationItemID;
            }
            return "";
        ...
        public static void AddVegetationItemInstance(string vegetationItemID, Vector3 worldPosition, Vector3 scale, Quaternion rotation, bool applyMeshRotation, byte vegetationSourceID, float distanceFalloff, bool clearCellCache = true)
        {
            ...
            if (Instance)
                for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                    if (Instance.VegetationSystemList[i].persistentVegetationStorage)
                        Instance.VegetationSystemList[i].persistentVegetationStorage.AddVegetationItemInstance(vegetationItemID, worldPosition, scale, rotation, applyMeshRotation, vegetationSourceID, distanceFalloff, clearCellCache);
        }
        ...
        public static void RemoveVegetationItemInstance(string vegetationItemID, Vector3 worldPosition, float minimumDistance, bool clearCellCache = true)
        {
            ...
            if (Instance)
                for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                    if (Instance.VegetationSystemList[i].persistentVegetationStorage)
                        Instance.VegetationSystemList[i].persistentVegetationStorage.RemoveVegetationItemInstance(vegetationItemID, worldPosition, minimumDistance, clearCellCache);
        }
        ...
        public static void RemoveVegetationItemInstance2D(string vegetationItemID, Vector3 worldPosition, float minimumDistance, bool clearCellCache = true)
        {
            ...
            if (Instance)
                for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                    if (Instance.VegetationSystemList[i].persistentVegetationStorage)
                        Instance.VegetationSystemList[i].persistentVegetationStorage.RemoveVegetationItemInstance2D(vegetationItemID, worldPosition, minimumDistance, clearCellCache);
        }
        ...
        public static void RemoveVegetationItemInstances(string vegetationItemID, byte vegetationSourceID)
        {
            ...
            if (Instance)
                for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                    if (Instance.VegetationSystemList[i].persistentVegetationStorage)
                        Instance.VegetationSystemList[i].persistentVegetationStorage.RemoveVegetationItemInstances(vegetationItemID, vegetationSourceID);
        }
        ...
        public static void RemoveVegetationItemInstances(string vegetationItemID)
        {
            ...
            if (Instance)
                for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                    if (Instance.VegetationSystemList[i].persistentVegetationStorage)
                        Instance.VegetationSystemList[i].persistentVegetationStorage.RemoveVegetationItemInstances(vegetationItemID);
        }
        ...
        public static void AddVegetationItemInstanceEx(string vegetationItemID, Vector3 worldPosition, Vector3 scale, Quaternion rotation, byte vegetationSourceID, float minimumDistance, float distanceFalloff, bool clearCellCache = true)
        {
            ...
            if (Instance)
                for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                    if (Instance.VegetationSystemList[i].persistentVegetationStorage)
                        Instance.VegetationSystemList[i].persistentVegetationStorage.AddVegetationItemInstanceEx(vegetationItemID, worldPosition, scale, rotation, vegetationSourceID, minimumDistance, distanceFalloff, clearCellCache);
        }
        ...
        public static List<VegetationPackagePro> GetVegetationPackageList(BiomeType biomeType)
        {
            ...
            for (int i = 0; i < Instance.VegetationSystemList.Count; i++)
                if (Instance.VegetationSystemList[i])
                {
                    VegetationPackagePro vegetationPackagePro = Instance.VegetationSystemList[i].GetVegetationPackageFromBiome(biomeType);
