...
namespace AwesomeTechnologies.Utility
{
    ...
    {
        public static unsafe void ClearMemory<T>(this NativeList<T> nativeList) where T : unmanaged
        {
            UnsafeUtility.MemClear(nativeList.GetUnsafePtr(), nativeList.Length * UnsafeUtility.SizeOf<T>());
        }

        public static unsafe void CompactMemory<T>(this NativeList<T> nativeList) where T : unmanaged
        {
            nativeList.Clear();
            nativeList.SetCapacity(0);
        }

        public static unsafe void CopyFromFast<T>(this NativeList<T> nativeList, T[] array) where T : unmanaged
        {
            if (array == null)
                throw new NullReferenceException(nameof(array) + " is null");

            int nativeArrayLength = nativeList.Length;
            int managedArrayLength = array.GetLength(0);
            if (managedArrayLength > nativeArrayLength)
                throw new IndexOutOfRangeException(nameof(nativeList) + " is shorter than " + nameof(array));

            int byteLength = managedArrayLength * UnsafeUtility.SizeOf<T>();
            void* managedBuffer = UnsafeUtility.AddressOf(ref array[0]);
            void* nativeBuffer = nativeList.GetUnsafePtr();

            UnsafeUtility.MemCpy(nativeBuffer, managedBuffer, byteLength);
        }

        public static unsafe void CopyFromFast<T>(this NativeList<T> nativeList, List<T> managedList) where T : unmanaged
        {
            if (managedList == null)
            ...
            if (managedListLength > nativeArrayLength)
                throw new IndexOutOfRangeException(nameof(nativeList) + " is shorter than " + nameof(managedInternalArray));
    ...
    public static class NativeArrayExtensions
    {
        public static unsafe void CopyToFast<T>(this NativeArray<T> nativeArray, T[] array) where T : struct
        {
            ...
            if (array.Length < nativeArrayLength)
                throw new IndexOutOfRangeException(nameof(array) + " is shorter than " + nameof(nativeArray));
        ...
        public static unsafe void CopyFromFast<T>(this NativeArray<T> nativeArray, T[,] array) where T : struct
        {
            ...
            if (managedArrayLength > nativeArrayLength)
                throw new IndexOutOfRangeException(nameof(nativeArray) + " is shorter than " + nameof(array));
        ...
        public static unsafe void CopyFromFast<T>(this NativeArray<T> nativeArray, List<T> managedList) where T : struct
        {
            ...
            if (managedListLength > nativeArrayLength)
                throw new IndexOutOfRangeException(nameof(nativeArray) + " is shorter than " + nameof(managedInternalArray));
